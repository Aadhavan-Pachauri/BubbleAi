...
export const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage, isLoading, chat, onChatUpdate, isAdmin, workspaceMode, isInitialView, loadingMessage, project, selectedAction, onActionSelect }) => {
  ...
  return (
    <div className="relative">
      <input type="file" ref={fileInputRef} onChange={handleFileChange} multiple style={{ display: 'none' }} accept="image/*" />
    ...
    >
        <div className="flex flex-col">
            <AnimatePresence>
                {(selectedAction !== 'default' || attachedFiles.length > 0) && (
                    <motion.div
                        // FIX: framer-motion props wrapped in a spread object to bypass type errors.
                        {...{
                        ...
                        className="w-full px-4 pt-3 flex items-center gap-2"
                    >
                        <div className="flex-shrink-0 flex items-center gap-1.5 px-2 py-1 bg-zinc-700/50 rounded-md text-sm text-zinc-200">
                            <ActionIcon className="w-4 h-4" />
                            <span>{currentActionDetails.name}</span>
                        </div>

                        {attachedFiles.length > 0 && (
                          <div className="flex-1 flex items-center gap-2 p-1.5 bg-zinc-700/50 rounded-lg min-w-0 overflow-x-auto">
                              {attachedFiles.map(file => (
                                  <div key={file.name + file.lastModified} className="relative flex-shrink-0">
                                      {file.type.startsWith('image/') ? (
                                        <img src={URL.createObjectURL(file)} alt="Preview" className="w-10 h-10 rounded-md object-cover" />
                                      ) : (
                                        <div className="w-10 h-10 flex items-center justify-center bg-zinc-600 rounded-md"><PaperClipIcon className="w-5 h-5"/></div>
                                      )}
                                      <button type="button" onClick={() => handleRemoveFile(file)} className="absolute -top-1.5 -right-1.5 p-0.5 bg-zinc-900 rounded-full text-zinc-400 hover:bg-zinc-600 hover:text-white transition-colors"><XMarkIcon className="w-4 h-4"/></button>
                                  </div>
                              ))}
                          </div>
                        )}
            ...
            <div className="flex items-center w-full p-2 gap-2">
               <button
                  ...
                  className="flex-shrink-0 w-8 h-8 rounded-full bg-zinc-700/50 flex items-center justify-center text-text-secondary hover:text-text-primary transition-colors"
              ...
              <textarea
                ...
                className="flex-1 bg-transparent text-text-primary placeholder-text-secondary focus:outline-none resize-none px-2 py-4 max-h-48"
                rows={1}
                disabled={isLoading}
